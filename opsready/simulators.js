(() => {
  ...
  const coreCount=document.querySelector('.control-metrics .metric-card:nth-child(2) strong');if(coreCount)coreCount.textContent='30';
  const coreSub=document.querySelector('.control-metrics .metric-card:nth-child(2) small');if(coreSub)coreSub.textContent='end-to-end production defects';
  ...
  function capture(){let f=$('controlFrame');if(!f)return;try{let doc=f.contentDocument,score=null,detail='';let actionScore=doc.getElementById('score'), actionMatch=actionScore&&actionScore.textContent.match(/(\d+)\s*\/\s*120/);if(actionMatch){score=Math.round(Number(actionMatch[1])/120*100);let card=doc.getElementById('scorecard');let unsafe=card&&card.textContent.match(/Unsafe actions:\s*(\d+)/i);detail='action score '+actionMatch[1]+'/120; unsafe '+(unsafe?unsafe[1]:'0')}else{score=0;detail='No graded action score found yet'}if($('labScore'))$('labScore').textContent=score+'%';if($('labScoreSub'))$('labScoreSub').textContent=detail;storeEvidence(controlTitle,score,detail);showToast(`${controlTitle}: ${score}% — ${detail}`)}catch(e){showToast('Complete/grade the lab first, then capture the score again.')}}
