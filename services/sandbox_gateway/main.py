...
from .models import Capability, CreateSessionRequest, ExecuteRequest, ExecuteResult, SessionInfo
...
origins = [x.strip() for x in os.getenv(
    "OPSREADY_CORS_ORIGINS",
    "http://localhost:3000,http://localhost:8080,http://127.0.0.1:5500,https://casino-snowflake-streaming-ai-platf-five.vercel.app",
).split(",") if x.strip()]
app.add_middleware(
    CORSMiddleware,
...
ADAPTERS = {"snowflake": snowflake, "databricks": databricks, "sqlserver": sqlserver}
...
def _get_session(session_id: str) -> Session:
    ...
    if not session:
        raise HTTPException(404, "Sandbox session not found or already cleaned up.")
    if time.time() - session.created_at > SESSION_TTL:
        ...
        raise HTTPException(410, "Sandbox session expired and cleanup was requested.")
    return session
...
def health() -> dict[str, object]:
    return {"ok": True, "service": "opsready-sandbox-gateway", "sessions": len(SESSIONS)}
...
def capabilities() -> list[Capability]:
    return [
        Capability(
            ...
            note="Real Snowflake SQL through snowflake-connector-python in an isolated training schema.",
        ),
        Capability(
            platform="databricks",
            ...
            note="SQL uses Statement Execution API; Python uses bounded Command Execution on a configured classic all-purpose cluster.",
        ),
        Capability(
            platform="sqlserver",
            ...
            note="Real T-SQL against an isolated SQL Server training database, intended for local/container sandbox use.",
        ),
    ]
...
def create_session(req: CreateSessionRequest) -> SessionInfo:
    ...
    if not adapter.available:
        raise HTTPException(503, f"{req.platform} is not configured on this gateway.")
    token = _short_id()
    ...
    try:
        ...
        raise HTTPException(502, f"Failed to create {req.platform} sandbox: {exc}") from exc
    session_id = secrets.token_urlsafe(24)
    SESSIONS[session_id] = Session(session_id, req.platform, namespace, req.admin_lab, time.time())
...
def execute(req: ExecuteRequest) -> ExecuteResult:
    ...
    if req.language == "python" and session.platform != "databricks":
        raise HTTPException(400, "Python execution is currently supported only for Databricks sessions.")
    ...
    try:
        if session.platform == "databricks":
            if req.language == "python":
                ...
                raw = databricks.execute_sql(session.namespace, req.code, admin_lab=session.admin_lab)
        else:
            raw = adapter.execute(session.namespace, req.code, admin_lab=session.admin_lab)
    except SandboxPolicyError as exc:
        raise HTTPException(403, str(exc)) from exc
    except Exception as exc:
    ...
    return ExecuteResult(
        ...
        validation={
            ...
            "policy_profile": "admin-lab" if session.admin_lab else "schema-safe",
...
def cleanup(session_id: str) -> dict[str, object]:
    ...
    try:
        ...
        SESSIONS.pop(session_id, None)
    return {"ok": True, "session_id": session_id, "platform": session.platform, "cleaned": True}
