...
class SnowflakeAdapter:
    ...
    def __init__(self) -> None:
        self.account = os.getenv("SNOWFLAKE_ACCOUNT", "") or os.getenv(
        ...
        self.allow_admin = (
            os.getenv("OPSREADY_ALLOW_ADMIN_LABS", "false").lower() == "true"
        )
    ...
    def create_session(self, schema: str, admin_lab: bool) -> str:
        if admin_lab and not self.allow_admin:
            raise SandboxPolicyError(
                "Snowflake admin labs are disabled on this gateway."
            )
    ...
    def execute(
        ...
        *,
        ...
        return AdapterResponse(
            ...
            rows=rows,
            message=(
                f"Executed {len(statements)} Snowflake statement(s) "
                f"in isolated schema {schema}."
            ),
...
class DatabricksAdapter:
    ...
    def available(self) -> bool:
        return bool(
            self.host and self.token and self.warehouse_id and self.catalog
        )
    ...
    def _sql(
        ...
        if state != "SUCCEEDED":
            ...
            raise RuntimeError(
                error.get("message")
                or f"Databricks statement ended in state {state}."
            )
    ...
    def create_session(self, schema: str, admin_lab: bool) -> str:
        if admin_lab and not self.allow_admin:
            raise SandboxPolicyError(
                "Databricks admin labs are disabled on this gateway."
            )
    ...
    def execute_sql(
        ...
        *,
        ...
        result.message = (
            f"Executed {len(statements)} Databricks SQL statement(s) "
            f"in {self.catalog}.{schema}."
        )
        return result
    ...
    def execute_python(self, schema: str, code: str) -> AdapterResponse:
        if not self.python_available:
            raise RuntimeError(
                "DATABRICKS_CLUSTER_ID is not configured for bounded Python execution."
            )
            ...
            try:
                command = client.post(
                    ...
                    json={
                        ...
                        "language": "python",
                        "command": (
                            f"spark.sql('USE CATALOG {self.catalog}')\n"
                ...
                if status.get("status") != "Finished":
                    cause = status.get("results", {}).get("cause")
                    raise RuntimeError(
                        cause
                        or "Databricks Python command did not finish successfully."
                    )
                results = status.get("results", {})
                text = (
                    results.get("data")
                    or results.get("summary")
                    or "Python command completed."
                )
            finally:
        ...
        return AdapterResponse(
            ...
            message=(
                "Databricks Python executed on bounded classic cluster context "
                f"in {self.catalog}.{schema}."
            ),
...
class SqlServerAdapter:
    ...
    def create_session(self, database: str, admin_lab: bool) -> str:
        if admin_lab and not self.allow_admin:
            raise SandboxPolicyError(
                "SQL Server admin labs are disabled on this gateway."
            )
        with self._connect("master") as conn:
            cur = conn.cursor()
            cur.execute(
                f"IF DB_ID(?) IS NULL EXEC('CREATE DATABASE [{database}]')",
                database,
            )
        return database
    ...
    def execute(
        ...
        *,
        ...
        return AdapterResponse(
            ...
            rows=rows,
            message=(
                f"Executed {len(statements)} SQL Server statement(s) "
                f"in isolated database {database}."
            ),
            elapsed_ms=int((time.perf_counter() - started) * 1000),
        )
    ...
    def cleanup(self, database: str) -> None:
        with self._connect("master") as conn:
            ...
            cur.execute(
                (
                    f"IF DB_ID(?) IS NOT NULL BEGIN ALTER DATABASE [{database}] "
